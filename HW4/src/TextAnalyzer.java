  ...
  public static class TextMapper extends Mapper<LongWritable, Text, Text, MapWritable> {
    ...
    throws IOException, InterruptedException {
      ...
      for(String word : wordSet.keySet()){
        ...
        // Create a copy of the master wordSet so that we don't get ConcurrentModificationExceptions
        TreeMap<String, IntWritable> wordSetCopy = new TreeMap<>(wordSet);

        // Create MapWritables containing key-value pairs <queryword, occurrence>
        MapWritable wordmap = new MapWritable();

        // Remove a single occurence of that word from the wordset
        int count = wordSetCopy.get(word).get()-1;
        if(count != 0){
          IntWritable tmpvalue = wordSetCopy.get(word);
          int count_val = tmpvalue.get() - 1;
          wordSetCopy.put(word, new IntWritable(count_val));
        }
        else
          wordSetCopy.remove(word);

        // Write to MapWritable
        for(Map.Entry<String, IntWritable> entry: wordSetCopy.entrySet())
          wordmap.put(new Text(entry.getKey()), entry.getValue());
